function OriRevCorrStimHartley_Adaptation_Version2(stim)
...
stim.DeltaStim = floor(stim.DeltaT * (scr.fps/stim.frPerScene));
...
ContLumChange = 0;
if sc > 1
    if diff([stim.AllStim(2,sc-1),stim.AllStim(2,sc)])~=0 || diff([stim.AllStim(3,sc-1),stim.AllStim(3,sc)])~=0
        ContLumChange = 1;
    end
end
% if ContLumChange==1
...
LumCont = dec2bin(0:(2^length(stim.LumList)-1)) - '0';
LumContCond = [];

c1 = 1;
for i = 1 : size(LumCont,1),
    for j = 1 : size(LumCont,1),
        if stim.SelectedLumCon(i,j)
            LumContCond(:,c1) = [LumCont(i,:) LumCont(j,:)];
            c1 = c1 + 1;
        end
    end
end
stim.LumContTrans = LumContCond;
stim.RepsMat = randi([1 size(LumContCond,2)],[1 ceil(stim.nScene/(2*stim.DeltaStim))]);
