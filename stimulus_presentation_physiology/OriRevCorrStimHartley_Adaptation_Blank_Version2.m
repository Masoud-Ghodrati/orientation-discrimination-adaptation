function OriRevCorrStimHartley_Adaptation_Blank_Version2(stim)
...
stim.RepsMat = randi([1 4],[1 4*stim.Reps]);
Pre_build_Stim=[];
...
for Allsc = 1 : length(stim.RepsMat)
    ...
    if Allsc>1
        for Scb = 1 : stim.DeltaStimBLK
            Pre_build_Stim=[Pre_build_Stim 3*ones(7,1)];
        end
    end
    
    switch stim.RepsMat(Allsc)
        ...
        case 1
            ...
            for Sc1 = 1 : stim.DeltaStim1,
                ...
                int_gauss = round(stim.Var(1)*randn+ceil(length( stim.VaritionRange)/2));
                while int_gauss<=0 || int_gauss>length( stim.VaritionRange)
                    int_gauss = round(stim.Var(1)*randn+ceil(length( stim.VaritionRange)/2));
                end
                cont = stim.VaritionRange(int_gauss);
                lum =  stim.LumList(1);
                Pre_build_Stim = [Pre_build_Stim [ori; cont; lum; phase; sff ; 1; 10*stim.RepsMat(Allsc)]];
                
            end
            for Sc1 = 1 : stim.DeltaStim2,
                
                ori = stim.oriList(randi(stim.nOri));
                phase = stim.phaseList(randi(stim.nPhase));
                sff = stim.sfPix(randi(stim.nsf));
                int_gauss = round(stim.Var(2)*randn+ceil(length( stim.VaritionRange)/2));
                while int_gauss<=0 || int_gauss>length( stim.VaritionRange)
                    int_gauss = round(stim.Var(2)*randn+ceil(length( stim.VaritionRange)/2));
                end
                cont = stim.VaritionRange(int_gauss);
                lum =  stim.LumList(1);
                Pre_build_Stim = [Pre_build_Stim [ori; cont; lum; phase; sff ; 2; 10*stim.RepsMat(Allsc)]];
        ...
        case 3
            for Sc1 = 1 : stim.DeltaStim1,
                ...
                lum = stim.VaritionRange(int_gauss);
                Pre_build_Stim = [Pre_build_Stim [ori; cont; lum; phase; sff ; 1; 10*stim.RepsMat(Allsc)]];
