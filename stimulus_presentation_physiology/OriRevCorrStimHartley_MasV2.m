function OriRevCorrStimHartley_MasV2(stim)
...
for Cont_n = 1 :  stim.nCont,
    for Ori_n = 1 : stim.nOri,
        for Phs_n = 1 : stim.nPhase,
            for SF_n = 1 : stim.nsf,
                
                ori = stim.oriList(Ori_n);
                cont = stim.contList(Cont_n);
                phase = stim.phaseList(Phs_n);
                sff = stim.sfPix(SF_n);
                
                a=[ori;cont;phase;sff];
                All_Stim=[All_Stim a];
...
Num_Blk = stim.nPhase * stim.nsf;

for Blk_n = 1 : Num_Blk
    a=[StimPerSeq;StimPerSeq;StimPerSeq;StimPerSeq];
    All_Stim=[All_Stim a];
end
...
%     cont = stim.contList(randi(stim.nCont));
